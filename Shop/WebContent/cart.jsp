...
<body>
	...
	<FORM name=cartForm action="<%=context %>/from/listaction!list"
		method=post>
		...
		<TABLE cellSpacing=0 cellPadding=0 width="75%" align=center border=1>
			<TBODY>
				...
				<c:forEach var="item" items="${sessionScope.cart.items}">
					<TR align=middle>
						<TD height=30><INPUT type=checkbox value=${item.key }
							name=itemCheck></TD>
						<TD height=30>${item.value.product.name}</TD>
						<TD height=30>${item.value.product.price}</TD>
						<TD height=30><INPUT maxLength=10 size=10
				...
				<TR>
					<TD colSpan=5 height=30>
						您的购物车中所有商品总金额：${sessionScope.cart.price }</TD>
				</TR>
			</TBODY>
		</TABLE>
		<BR> <BR>
		<TABLE cellSpacing=0 cellPadding=0 width=450 align=center border=0>
			<TBODY>
				<TR align=middle>
					<TD width="22%"><INPUT onclick=modifyNumber() type=button
						value=修改所选项的数量 name=Button></TD>
					<TD width="20%"><INPUT onclick=deleteCartItem() type=button
						value=删除所选项 name=Submit2></TD>
					<TD width="26%"><INPUT onclick="clearCart();" type=button
						value=清空购物车 name=Submit3></TD>
					<TD width="12%"><INPUT
						onclick="javascript:location.href='order_index?method=index'"
						type=button value=结账 name=Submit42></TD>
					<TD width="20%"><INPUT
						onclick="javascript:location.href='listaction?method=list'"
						type=button value=继续购物 name=Submit42></TD>
				</TR>
			</TBODY>
		</TABLE>
		<P align=center>&nbsp;</P>
