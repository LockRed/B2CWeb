...
<body>
	<H1 align=left>
				...
				<s:iterator value="productlist" var="p">
					<TR>
						<TD>
							...
							<!-- 2014.9.2 配置server。xml  客户端使用相对地址 --> <img
						...
						<TD><a
							href="<%=context %>/manager/update?productId=<s:property value="#p.id"/>">修改商品</a>
							<%-- <a href="<%=context %>/admin/Category_updateinput?id=<s:property value="#c.id"/>">修改Category</a> --%>
						</TD>

						<TD><a
							href="<%=context %>/manager/delete?productId=<s:property value="#p.id"/>">删除商品</a>
							<%-- <a href="<%=context %>/admin/Category_updateinput?id=<s:property value="#c.id"/>">修改Category</a> --%>
						</TD>
		...
		<s:if test="#page>1 ">
			<a href="<%=context %>/manager/listproduct?page=1"> 首页</a>
		</s:if>

		<s:if test="#page == 1 ">上一页</s:if>

		<s:if test="#page>1 ">
			<a href="<%=context %>/manager/listproduct?page=${page-1}"> 上一页</a>
		</s:if>

		<s:if test="#page ==#maxpage">下一页</s:if>

		<s:if test="#page< #maxpage ">
			<a href="<%=context %>/manager/listproduct?page=${page + 1}"> 下一页</a>
		</s:if>

		<s:if test="#page == #maxpage">末页</s:if>

		<s:if test="#page< #maxpage  ">
			<a href="<%=context %>/manager/listproduct?page=${maxpage}">末页</a>
		</s:if>

		<BR> <a href="<%=context %>/manager/index.jsp">返回主页</a>
