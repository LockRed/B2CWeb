...
<body>
		...
		<TBODY>
			...
			<s:iterator value="orders" var="o">
				<TR>
					...
					<TD><c:if test="${o.status != 4}">

							<a
								href="<%=context %>/manager/nextaction!next?orderId=<s:property value="#o.id"/>">下一步</a>

						</c:if> <%-- <a href="<%=context %>/admin/Category_updateinput?id=<s:property value="#c.id"/>">修改Category</a> --%>
					</TD>
				</TR>
			</s:iterator>
		</TBODY>
	...
	<center>
		...
		<a href="<%=context %>/from/loginoutaction">退出<a> <a
				href="<%=context %>/manager/index.jsp">返回主页</a>
	</center>
</BODY>
